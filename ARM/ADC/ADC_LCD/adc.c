...
unsigned long Read_ADC0( unsigned char channelNum )
{
    ...
    AD0CR &= 0xFFFFFF00;             // Clear all SEL bits 
	
    AD0CR |= (1 << 24) | (1 << channelNum);	 /* switch channel, start A/D convert */
	...
	while ( 1 )	{		                   /* wait until end of A/D convert */
		...
		regVal = AD0GDR;                 //		regVal = *(volatile unsigned long *)(AD0_BASE_ADDR + ADC_INDEX);
    ...
    if ( regVal & ADC_OVERRUN )	{   /* save data when it's not overru otherwise, return zero */	    
		return ( 0 );
    }
